...
Baloto draws 5 main balls from 1-43 plus one "superbalota" from 1-16,
every Wednesday and Saturday. Every module that needs those bounds or
needs to turn `balls_expanded` (from utils.processor.load_and_preprocess)
into one series per ball position imports from here so the ranges only
live in one place.
"""
...
def next_draw_dates(last_date, h):
    """Next h Wednesday/Saturday dates after last_date, matching Baloto's real draw calendar."""
    dates = []
    cursor = last_date
    while len(dates) < h:
...
def to_long_format(position_series):
    ...
    for position, frame in position_series.items():
        ...
        f["ds"] = range(1, len(f) + 1)  # sequential draw index; calendar gaps are irregular (Wed/Sat only)
        frames.append(f)
