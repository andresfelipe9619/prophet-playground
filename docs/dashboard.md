...
The calibration block under the verdict is fed from `table.attrs["forecasts"]`,
which `compare_models` attaches, so asking this second question of the same run
refits nothing. It draws a reliability curve for every scored model **and the
market**, an error table with a `coverage` column (a small error over a tenth of
the forecasts is not a small error), and the per-outcome calibration-in-the-large
table with its corrected verdict. The market usually posts the lowest error on
that table, which is the sanity check: it is the thing everything else is being
measured against.
